...
def worker_api_get_json(endpoint: str, timeout: int = 60) -> dict[str, object] | None:
    url = f"{worker_api_base_url()}{endpoint}"
    result = run_curl(["--max-time", str(timeout), url], timeout=timeout + 15)
    if result.returncode != 0:
        return None
    try:
        payload = json.loads(result.stdout)
...
def get_required(slug: str) -> str:
    ...
    raise RuntimeError(
        f"Memory Stargraph HTTP raw read and gbrain get failed for {slug}: {result_error(result)}"
    )
...
def put_readback(slug: str, markdown: str) -> str:
    ...
    if not payload or payload.get("error"):
        ...
        if result.returncode:
            raise RuntimeError(
                f"Memory Stargraph HTTP save and gbrain put failed for {slug}: {result_error(result)}"
            )
...
def graph_edges(source: str, relation: str) -> list[dict[str, object]]:
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Memory Stargraph HTTP graph-query and gbrain graph failed for {source}: {result_error(result)}"
        )
    try:
...
def link(source: str, target: str, relation: str) -> None:
    ...
    if not payload or payload.get("error"):
        result = run_gbrain(
            [
                "link", source, target, "--link-type", relation,
                "--link-source", "memory-stargraph-capture-backlog",
            ]
        )
...
def unlink(source: str, target: str, relation: str) -> None:
    ...
    if not payload or payload.get("error"):
        result = run_gbrain(
            [
                "unlink", source, target, "--link-type", relation,
                "--link-source", "memory-stargraph-capture-backlog",
            ]
        )
