...
try {
  ...
  if (initial.lastRefreshPresent || initial.refreshInlineControls || JSON.stringify(initial.settingsSections) !== JSON.stringify(["Display", "Yoda", "Setup", "Review", "Cache"]) || !initial.flushCachePresent || !initial.selectionYodaPresent) {
    throw new Error(`Expected Settings Display/Yoda/Setup/Review/Cache with Flush, no refresh controls, no Last refresh, and selection Yoda button: ${JSON.stringify(initial)}`);
  }
