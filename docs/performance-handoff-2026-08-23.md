...
### Dedicated flowing-particle Canvas layer

Commit `31afc98` places the retained flowing particles on a pointer-transparent middle Canvas, between the static background/edge Canvas and the existing node Canvas. Static radar, stars, clouds, base edges, and relationship labels now redraw only on dirty interaction, projection movement, or the existing 15 FPS static cadence. The particle layer clears and redraws independently on each animation callback, while one sorted edge list is shared across all layers. Paint order remains static edges, moving particles, then nodes and labels.

- Four same-server before/after/after/before pages each ran five forced 60-frame batches with every Canvas forced through rasterization. Aggregate per-page batch median fell from 3,600.0 to 1,343.3 milliseconds, a 62.7% reduction. Complete dirty-frame median also improved from 66.8 to 57.8 milliseconds, or 13.5%.
- An eight-page direction-balanced real `requestAnimationFrame` observation retained comparable three-second frame throughput while reducing total callback work. Conventional median callbacks increased from 50 to 53, total callback time fell from 280.0 to 252.8 milliseconds (9.7%), static main-layer clears fell from 50 to 30.5 (39.0%), and callback p95 fell from 11.75 to 9.65 milliseconds (17.9%).
- Every sample retained 274 nodes, 139 edges, and 48 animated particles with no runtime errors or overflow. Turning Flow off cleared the dedicated layer from 14,149 nontransparent pixels to zero and released the animation loop.
...
### Validated Brotli startup sidecars

The HTML, CSS, and JavaScript startup assets now negotiate prebuilt Brotli quality-5 sidecars before the existing gzip fallback. A committed manifest records every source SHA-256, source size, and compressed size. The server validates those fields before sending `br`; missing, malformed, stale, or size-mismatched artifacts fail closed to gzip or the standard plain-file path. This adds no runtime compression dependency or request-time Brotli CPU.

- Encoded startup text fell from 119,045 gzip bytes to 85,343 Brotli bytes, a 28.3% reduction. HTML changed from 6,436 to 5,133 bytes, CSS from 22,564 to 15,371 bytes, and JavaScript from 90,045 to 64,839 bytes.
- An eight-page direction-balanced Chrome A/B used fresh browser contexts at 5 Mbps and 80 milliseconds of latency. Median `DOMContentLoaded` fell from 557.2 to 488.8 milliseconds, a 12.3% improvement, while all decoded byte counts remained exact and there were no page or console errors.
- `scripts/build_static_brotli.py` is the deterministic update path. Static tests verify all three source hashes and sizes against the manifest, compressed-file sizes, encoding quality, Brotli preference, lossless gzip fallback, conditional requests, and stale-sidecar fallback. Manifest mtime and size participate in the in-process validation cache key, so a rebuilt sidecar is discoverable without serving stale bytes. Cold JS sidecar validation took 1.39 milliseconds; warm validation had a 0.016-millisecond median and 0.038-millisecond p95. Forty local HTTP samples per encoding had the same 1.63-millisecond median for Brotli and gzip. The full suite passed 654 tests in 43.707 seconds before the final quality-value assertion was added.
...
- The representative `products/memory-stargraph` selection previously transferred a 1,645,179-byte, 1480 by 1030 PNG into a 272-pixel desktop panel and 70-pixel mobile panel. Its final low-effort preview is a 35,310-byte, 640 by 445 WebP, a 97.85% transfer reduction.
- An initial three-round direction-balanced Chrome A/B at 5 Mbps and 80 milliseconds reduced selection-media completion median from 3,190.8 to 230.0 milliseconds, a 92.8% improvement. The media-resource median fell from 3,113.6 to 145.7 milliseconds.
- The initial higher-effort encoder took 174.3 milliseconds for a cold representative preview through the isolated HTTP server; its immediate cached request took 1.9 milliseconds and returned byte-identical output. The original media endpoint remained unchanged.
...
Enabling HTTP/1.1 keep-alive on the local request handler was rejected without changing source. A six-pair direction-balanced Chrome A/B used the existing TLS certificate on two isolated services at 5 Mbps and 80 milliseconds of latency. HTTP/1.1 reduced median startup connections from 12 to 5 and five serial API calls from 4 connections to 1, but user-visible graph readiness improved only 4.0%. The serial API median improved from 695.3 to 595.2 milliseconds, or 14.4%, just below the 15% acceptance threshold. All requests returned `200`, both variants rendered 185 nodes, and there were no page errors. The protocol override, static contract test, and isolated TLS services were fully reverted; revisit only with a representative workflow that demonstrates a material latency win.

Starting `/api/hidden` and `/api/graph` concurrently during browser initialization was rejected without changing source. A six-pair direction-balanced Chrome A/B used fresh contexts at 5 Mbps and 80 milliseconds of latency against two isolated same-data services. Concurrency reduced the median request-start gap from 108.9 to 0.9 milliseconds, but graph response completion improved only 12.6% and user-visible graph readiness improved from 856.2 to 754.8 milliseconds, or 11.8%, below the 15% acceptance threshold. Both variants rendered exactly 185 nodes with no page errors. The concurrent startup, late-hidden render guard, static assertions, generated Brotli change, temporary services, config files, and worktree were fully reverted.
...
Pre-rasterizing the two flowing-particle glow styles as DPR-aware offscreen sprites was rejected. Four same-server particle-only pages retained 274 nodes, 139 edges, and 48 particles. Direct small-radius shadow paths had steady medians of 1.9 and 3.1 milliseconds; the sprite `drawImage` candidate took 98.1 and 90.9 milliseconds, a roughly 30-50x regression. The full fixed-frame run was stopped after more than 180 seconds versus its normal roughly 45-60 seconds. Chrome optimizes these tiny shadows much better than repeated scaled-bitmap composition, so the sprite cache and draw path were fully reverted.
...
Compact JSON separators and direct UTF-8 output were rejected without changing source. Across Graph, Chinese Search, Entity, and Settings payloads, compact UTF-8 reduced uncompressed bytes by 4.8-8.7%, but the already accepted gzip layer reduced the incremental wire savings to only 1.1-2.5%. Compression CPU was unchanged at 0.07-0.35 milliseconds. Changing response formatting for a sub-threshold transfer gain was not justified.

Removing common English and Chinese request prefixes from primary Search cache keys was rejected without changing source. Ten plain versus prefixed pairs covered `find`, `show me`, `search for`, `查找`, and `搜索`. Top-ten order matched exactly in 0/10 pairs; Jaccard overlap ranged from 0.25 to 0.82, and the Chinese backup query changed its first result. These words currently carry retrieval semantics in GBrain, so cross-prefix cache reuse would return incorrect rankings.

Reducing only the background and flowing-particle Canvas layers from the accepted DPR 2 cap to DPR 1.5 was rejected without changing source. The node and label layer stayed at DPR 2, while the other two backing stores changed from 1,700 by 1,040 to 1,275 by 780 pixels. Across six direction-balanced pages and twelve forced-raster batches per variant, aggregate median improved from 1,381.3 to 1,212.5 milliseconds, only 12.2% and below the 15% acceptance threshold. Mean improved 16.9%, but per-page medians were directionally inconsistent. The smaller underlays would also soften edge and glow detail, so all three layers retain the simpler common DPR 2 cap.

Lowering the common high-density Canvas cap again from DPR 2 to DPR 1.75 was rejected without changing source. Although each layer's backing store fell from 1,700 by 1,040 to 1,488 by 910 pixels, six direction-balanced pages regressed the twelve-batch median from 1,307.3 to 1,383.4 milliseconds (5.8%) and the mean from 1,300.7 to 1,364.6 milliseconds (4.9%). As with prior cloud-radius and outline experiments, Chrome's software Canvas raster cost is not monotonic with fewer pixels. Keep the measured DPR 2 cap rather than tuning resolution by arithmetic alone.

Moving the startup script from the end of the document to a deferred `<head>` script was rejected without changing source. A controlled 5-Mbps, 80-millisecond-latency Chrome comparison used isolated services at the same commit. Because compressed HTML is only about 6.4 KB, Chrome discovered the current tail script and the candidate head script at effectively the same time: CSS and JavaScript request starts differed by roughly one millisecond within each sample. Candidate `DOMContentLoaded` showed no improvement and ordinary timing variance dominated. The temporary service and worktree were removed after the test.
...
Forcing the four cold Search evidence-list reads onto independent GBrain subprocesses was rejected. A corrected fresh-process A/B waited for the persistent lane to be ready and unlocked before each sample. The current lane completed primary Search plus four serialized evidence lists in 1.853 and 0.603 seconds with complete evidence. The isolated-list candidate took 3.769 and 4.138 seconds; the latter timed out all four evidence lists and returned `partial_timeout`. The healthy persistent process showed that each list added only about 20-40 milliseconds after primary retrieval, while four independent process startups duplicated backend initialization and changed evidence ordering. Keep primary and evidence reads on the same persistent lane; investigate long-lived fallback state separately if it recurs.
