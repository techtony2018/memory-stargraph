...
<p align="center">
  <img src="docs/assets/memory-stargraph-ui.png" alt="Memory Stargraph interactive star-cloud UI showing clustered gbrain entities, relationship hover, Ask Yoda, Autopilot, and local cache controls">
</p>
...
Ask Yoda is a node-scoped chat workflow. When you open Ask Yoda from a selected node, the browser sends the node slug, current question, recent chat history, and configured Yoda depth to the local Memory Stargraph server. Yoda depth controls graph-query hop depth and how many related source nodes the server reads directly: lower values are faster, higher values provide broader graph context. The depth control is exposed both in the Ask Yoda chat and in Settings, with both controls staying synchronized.

Under the hood, the server gathers the selected page with `gbrain get`, nearby relationships with graph/backlink queries, targeted search snippets, media hints, and direct reads of likely related source nodes. For relationship questions that name another entity, it also resolves that entity, reads its backlinks, and follows matching relationship sources such as `reposted_by` directly. Short follow-ups such as “try again” inherit the latest substantive user question for retrieval, while prior assistant answers remain non-authoritative conversation context. Targeted relationship lookups constrain broad graph expansion to avoid high-degree hub timeouts. Broad graph output is explicitly marked as possibly truncated, so absence from a clipped container listing is never treated as proof that an edge does not exist. If the configured agent path is available, Memory Stargraph asks that local agent to produce the response. If the agent is unavailable or times out, the endpoint falls back to a concise GBrain-context response so the UI still gives an actionable answer.
