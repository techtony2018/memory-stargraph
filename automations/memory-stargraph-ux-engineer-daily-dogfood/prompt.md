...
3. Verify `http://127.0.0.1:8788/api/health`; record `ui_version`, source state, warnings, and attachments. Confirm there is no active Engineer active-change marker and record a deployment fingerprint containing at least `ui_version`, health source state and timestamp, served HTML/JS asset version or hash, and local process cwd when available. Final evidence must use dashboard-managed `http://127.0.0.1:8788`, not a temporary service or source inspection.
...
9. Perform these checks before and after every journey: re-read the Engineer active-change marker, health, and complete deployment fingerprint. If a marker appears, health becomes unstable, or any fingerprint field changes, stop immediately, discard all observations from the unstable run, create or update no TODOs, terminalize the active UX Run/lease as `deferred_due_to_active_change`, record the before/after evidence, and report that a future invocation should retry. Do not carry an unstable observation into a later run.
