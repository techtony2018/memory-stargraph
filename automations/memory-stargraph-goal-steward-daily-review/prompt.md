...
5. Do not rely on workers being able to call cross-thread messaging tools. At the start of every full review and any worker-watch recovery cleanup, sweep the latest terminal Run/report evidence for `product_owner_notification_pending: true` or `product_owner_notification_status: pending_unacknowledged_delivery` across the watched roles. Recover the compact payload from the worker Run/report, verify the worker outcome, and update both Run and report to `product_owner_notification_status: acknowledged_by_product_owner` and `product_owner_notification_pending: false` before counting the role as healthy or dispatching a dependent retry.
6. If a worker says direct Product Owner messaging was unavailable, exposed but not callable, or unacknowledged, classify it as an automation-governance gap only when the Run/report lacks the full compact payload or Product Owner sweep cannot acknowledge it. Otherwise treat it as expected fallback behavior and complete the PO reconciliation immediately.
